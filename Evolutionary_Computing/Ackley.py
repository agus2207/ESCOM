...
def f(x):
    term1 = - 20 * math.exp(-0.2 * math.sqrt(0.5 * (abs(x**2)+abs(2**2))))
    term2 = - math.exp(0.5 * (math.cos(2 * math.pi  * x)+math.cos(2 * math.pi  * 2)))
...
def graph_f():
    ...
    for i in range(1,N):
        xnew=xini+i*dx
        ynew=f(xnew)
        w.create_line(xo+xold*s,yo-yold*sy,xo+xnew*s,yo-ynew*sy)
        xold=xnew
        yold=ynew
