...
void resultado(double a[], int b[], int n){
...
void imprimir(double a[], int b, int c){ //Funcion que imprime los arreglos
	for(int i = 0; i <b; i++){
		printf("A[%d] = %.2f\n", c, a[i]);
		c=c+1;
	}
...
double convolucion(double espejo[], double x[], int n, int d, int taml, int tamx, int negl, int negx){ //Funcion que relaiza la convolucion discreta
	int des;
	double res = 0.0; //Variables que guardaran el resultado y el desplazamiento en la LIT invertida
	/*double espejo[taml]; //Declaramos un arreglo donde se almacenara la inversion de LIT
...
void llenar (double a[], int b){ //Funcion que llena los arreglos
	double valor;
	for (int i = 0; i < b; i++){ 
		 scanf("%lf",&valor);
		 a[i] = valor;
...
void inicio(){ //Funcion que inicia el programa
	...
	double res[n], espejo[size1]; 
	int desp[n];
	neg1=(-1)*neg1+1; //Volvemos positivo el valor mas pequeño en el eje de las x de LIT
	for(int i=0; i<size1; i++){ //Invertimos LIT
		espejo[size1-i-1]=LIT[i]; //El arreglo que almacena la inversion de LIT se llama espejo
		neg1=neg1-1; //Invertimos todos los valores del eje de x en LIT
	}
	printf("<---h(-k)--->: \n"); //Se imprime la señal de entrada en pantalla
	imprimir(espejo, size1, neg1);
	for(int i=0; i<n; i++){ //Recorre y llena el arreglo de la respuestas de la convolucion
		...
		scanf("%d", &des);
		res[i]= convolucion(espejo, x, n, des, size1, size2, neg1, neg2); //Manda a llamar a la funcion concolucion para calcular la respuesta
		printf("y[%d] = %.2f \n", des, convolucion(espejo, x, n, des, size1, size2, neg1, neg2)); //Se muestra la respuesta en pantalla
		desp[i] = des;
	}
